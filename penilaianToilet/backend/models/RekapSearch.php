<?php

namespace backend\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use common\models\rekap;

/**
 * RekapSearch represents the model behind the search form of `common\models\rekap`.
 */
class RekapSearch extends rekap
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id_rekap', 'id_daop', 'id_kantor', 'id_station', 'id_kereta', 'id_toilet', 'id_respon', 'id_ketidakpuasan'], 'integer'],
            [['created_at', 'created_by', 'bulan', 'tahun','tanggal'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = rekap::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id_rekap' => $this->id_rekap,
            'created_at' => $this->created_at,
            'id_daop' => $this->id_daop,
            'id_kantor' => $this->id_kantor,
            'id_station' => $this->id_station,
            'id_kereta' => $this->id_kereta,
            'id_toilet' => $this->id_toilet,
            'id_respon' => $this->id_respon,
            'id_ketidakpuasan' => $this->id_ketidakpuasan,
        ]);

        $query->andFilterWhere(['like', 'created_by', $this->created_by])
            ->andFilterWhere(['like', 'bulan', $this->bulan])
            ->andFilterWhere(['like', 'tahun', $this->tahun])
            ->andFilterWhere(['like', 'tanggal', $this->tanggal]);
        return $dataProvider;
    }
}
